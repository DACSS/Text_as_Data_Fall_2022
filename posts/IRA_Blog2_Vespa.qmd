...
```{r}
# Read in data
R.utils::getAbsolutePath("IRA_med.csv")
IRA.csv <- readtext("C:/Users/Owen/Desktop/Text_as_Data_Fall_2022/IRA_med.csv")
```
...
```{r}
IRA_text <- IRA_df %>%
  select ("text.1", created_date) #new dataframe isolate text column 
```

remove @twitter handles

```{r}
IRA_text$text <- gsub("@[[:alpha:]]*","", IRA_text$text.1) #remove Twitter handles
```
