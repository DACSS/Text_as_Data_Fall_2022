...
create_corpus_occupation <- tm_map(create_corpus, content_transformer(tolower))
create_corpus_occupation <- tm_map(create_corpus, removeWords, stopwords("english"))
...
![**Word Cloud of Occupations**](AdithyaParupudi_people_titles.png)

Interesting to see what TIME means here. Just like we see below, there are many born in the warring ages which shaped many people's careers and their deeds, thus landing an entry in this list.
...
create_corpus <- Corpus(VectorSource(bio$word))
...
![**People's Biographies**](AdithyaParupudi_contentCloud.png)
